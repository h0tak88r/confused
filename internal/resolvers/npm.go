...
// isVulnerablePackage checks if a package is vulnerable to dependency confusion
func (n *NPMLookup) isVulnerablePackage(pkg NPMPackage) bool {
	// Skip local, URL, and git references
	if n.localReference(pkg.Version) || n.urlReference(pkg.Version) || n.gitReference(pkg.Version) {
		return false
	}
	...
	if n.gitHubReference(pkg.Version) {
		return !n.gitHubOrgExists(pkg.Version)
	}
	
	// Check if available in public registry
	return !n.isAvailableInPublic(pkg.Name, 0)
}